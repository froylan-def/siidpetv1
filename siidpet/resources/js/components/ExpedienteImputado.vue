  ...
  <div :class=claseEstados[0]> 
     <div class="card-header">
      ...
      <div v-if="estados[0] != 0" class="card-tools">
        ...
        <button type="button" class="btn btn-tool" data-card-widget="maximize" title="Collapse">
    ...
    <div class="card-body p-0">
      ...
      <div class="  col-md-9 mb-2 w-100">
          <button  class="btn btn-primary"   @click="finalizarEtapa(1)" >
        ...
        <div class="form-group">
          <div class="col-md-9 mb-2 w-100">
            ...
            <div v-if="estados[0] == 1">  
              ...
              <textarea name="Text1"  v-model="nota.texto"  class="form-control" cols="70" rows="5"></textarea>
              <button  class="btn btn-primary"   @click="registrarNota(1)" >
  ...
  <div :class=claseEstados[1]>
    ...
    <div class="card-body table-responsive p-0">
      <div class="form-group">
        ...
        <div class="  col-md-9 mb-2 w-100">
        <br>
               <button  class="btn btn-primary"   @click="finalizarEtapa(2)" >
                <i class="fa-solid fa-eye"></i> Continuar
              </button>  
            </div>  
          <h5 >Bitacora</h5> 
          <div v-if="estados[1] == 1">  
              <label >Agregar nota</label>
              <textarea name="Text1"  v-model="nota.texto"  class="form-control" cols="70" rows="5"></textarea>
              <button  class="btn btn-primary"   @click="registrarNota(2)" >
                <i class="fa-solid fa-eye"></i> Agregar
              </button>  
            </div>
...
export default 
{
    ...
    methods: 
    {
      ...
      getList() 
      {
        this.axios.get('/bitacora/notas/'+this.idimputado).then((response) => 
        {
      ...
      getEstadoEtapas() 
      {
        this.axios.get('/etapa/imputado/'+this.idimputado).then((response) => 
        {
          this.estados=response.data
          for (let i = 0; i <  this.estados.length; i++) 
          {
            if(this.estados[i]==0)
              this.claseEstados[i]= "card collapsed-card card-secondary"
            if(this.estados[i]==1)
              this.claseEstados[i]= "card card-primary"
            if(this.estados[i]==2)
              this.claseEstados[i]= "card collapsed-card card-success"
          }
        })
      },  
      async registrarNota(idEtapa) 
      {
        ...
        await this.axios.post("/bitacora", this.nota).then((response) => 
        {
          this.getList();
      ...
      async finalizarEtapa(idEtapa) 
      {
        this.nota.idimputado=Number(this.idimputado);
        this.nota.idEtapa=Number(idEtapa);
        await  this.axios.post("/bitacora", this.imputado).then((response) => 
        {
          this.getList();
          console.log(response);
        }).catch(error => 
